# SendRecv · busBw 推导与手算

> 源码位置：`rccl-tests/src/sendrecv.cu` 第 39-45 行
...
<div class="bw-root">
    <div class="bw-title">SendRecv · busBw 理论上限推导</div>
    <div class="bw-sub">源码 sendrecv.cu: factor = 1，n = 8 GPU，M = 128 MB</div>
    <div class="bw-grid">
      <div class="bw-ring">
        <svg viewBox="0 0 300 340" width="300" height="340" xmlns="http://www.w3.org/2000/svg">
          ...
          <!-- 环形连线 G0->G1->G2->G3->G4->G5->G6->G7->G0 -->
          ...
          <!-- M=128MB 标注（G0->G1 段外侧） -->
          <g>
            <rect x="226" y="191" width="72" height="18" rx="6" fill="#efebff" stroke="#7c5cff" stroke-width="1"/>
            <text x="262" y="200" text-anchor="middle" dominant-baseline="central" font-size="11" fill="#1a1a2e">M=128MB</text>
          </g>
          <!-- 底部标注 -->
          <g>
            <rect x="66" y="302" width="168" height="24" rx="6" fill="#efebff" stroke="#7c5cff" stroke-width="1"/>
            <text x="150" y="314" text-anchor="middle" dominant-baseline="central" font-size="12" fill="#1a1a2e">点对点收发 · T = M/B</text>
          </g>
        </svg>
      </div>
      <div class="bw-derive">
        <div class="bw-derive-head">推导链</div>
        <div class="bw-step"><span class="n">1</span><span class="t">M = 128 MB, n = 8</span><span class="d">每 rank 向 peer 发送 128MB，接收 128MB</span></div>
        <div class="bw-step"><span class="n">2</span><span class="t">每 rank 经一条链路发送 M</span><span class="d">全双工下发送与接收可并行</span></div>
        <div class="bw-step"><span class="n">3</span><span class="t">T = M / B</span><span class="d">单链路传输时间</span></div>
        <div class="bw-step"><span class="n">4</span><span class="t">algBw = M / T = B</span><span class="d">算法带宽等于链路带宽</span></div>
        <div class="bw-step"><span class="n">5</span><span class="t">busBw = algBw × 1 = B ✓</span><span class="d">factor=1，纯点对点无集合开销</span></div>
        <div class="bw-concl">
          <div class="k">busBw = B</div>
          <div class="h">SendRecv 是所有集合操作的原子基线，复杂集合可分解为多次 SendRecv 编排。与 Broadcast/Reduce 同属 factor=1 单链路瓶颈类。</div>
        </div>
        <div class="bw-legend">
...
```c
void SendRecvGetBw(size_t count, int typesize, double sec,
...
- `count` = paramcount = M（每 rank 发送 M 给 peer，接收 M 来自另一 peer）
- `baseBw` = M / T
- `factor` = 1 → **busBw = algBw**

## 二、参数含义（用户 -b 128MB, n = 8）

SendRecvGetCollByteCount 中 sendcount = recvcount = count。每 rank 向 `(rank-1+n)%n` 发送、从 `(rank+1)%n` 接收（环形相邻配对）。

| 量 | 计算 | 值 |
|----|------|-----|
| count (paramcount) | 用户 -b | 128 MB |
| sendcount / rank | = count | 128 MB（发给 peer）|
| recvcount / rank | = count | 128 MB（收自 peer）|

## 三、算法推导（点对点）

SendRecv 是最简单的集合操作：每 rank 与一个 peer 做双向收发。本质是点对点传输。
...
- 每 rank 发送 M = 128MB 给 peer，经一条链路
- 双向链路（全双工）下，发收可并行，T = M/B
- 总时间：
...
**busBw 理论上限 = B（单向链路带宽），与 n 无关。**

- SendRecv 是纯点对点，无集合算法开销，busBw = algBw = B
- factor = 1，因为没有多 rank 数据复用需要归一化
- 它是所有集合操作的"原子操作"基线：复杂集合操作可分解为多次 SendRecv 的编排

> **与 Broadcast/Reduce 的共性**：三者 factor 均为 1，都是"单链路瓶颈"型——SendRecv 是相邻 peer 间单链路，Broadcast/Reduce 是 root 单链路。busBw = algBw，上限直接由瓶颈链路决定。
