...
> 源码位置：`rccl-tests/src/gather.cu` 第 39-45 行
...
<div class="bw-root">
    <div class="bw-title">Gather · busBw 理论上限推导</div>
    <div class="bw-sub">源码 gather.cu: factor = (n−1)/n，n = 8 GPU，M = 128 MB</div>
    <div class="bw-grid">
      <div class="bw-ring">
        <svg viewBox="0 0 300 340" width="300" height="340" xmlns="http://www.w3.org/2000/svg">
          ...
          <g stroke="#7c5cff" stroke-width="2" fill="none">
            <line x1="219" y1="84"  x2="76"  y2="161" marker-end="url(#ah)"/>
            <line x1="251" y1="122" x2="77"  y2="166" marker-end="url(#ah)"/>
            <line x1="260" y1="170" x2="78"  y2="170" marker-end="url(#ah)"/>
            <line x1="251" y1="218" x2="77"  y2="174" marker-end="url(#ah)"/>
            <line x1="219" y1="256" x2="76"  y2="179" marker-end="url(#ah)"/>
            <line x1="175" y1="280" x2="73"  y2="182" marker-end="url(#ah)"/>
            <line x1="129" y1="283" x2="69"  y2="185" marker-end="url(#ah)"/>
          </g>
          ...
          <!-- root 标注 收 8×16MB=128MB（G0 上方两行） -->
          <g font-size="10" fill="#5b3fd6" text-anchor="middle" dominant-baseline="central">
            <text x="60" y="128">收 8×16MB</text>
            <text x="60" y="142">= 128MB</text>
          </g>
          <!-- 节点 -->
          ...
          <g>
            ...
            <text x="150" y="329" text-anchor="middle" dominant-baseline="central" font-size="11" fill="#1a1a2e">根扇入 · 7 × 16 MB</text>
          </g>
        </svg>
      </div>
      <div class="bw-derive">
        <div class="bw-derive-head">推导链</div>
        <div style='font-family:"SF Mono","JetBrains Mono",Menlo,monospace;font-size:11px;color:#6b6b80;margin-top:-4px;'>源码 gather.cu: baseBw = count·nranks·typesize/1e9/sec · factor = (n−1)/n · busBw = baseBw·factor</div>
        <div class="bw-step"><span class="n">1</span><span class="t">M = 128 MB, n = 8</span><span class="d">每 rank 贡献 base = M/n = 16MB，root 收集 128MB</span></div>
        <div class="bw-step"><span class="n">2</span><span class="t">(n−1)·base = 7×16MB</span><span class="d">7 个其他 rank 各向 root 发 base，root 共收 112MB</span></div>
        <div class="bw-step"><span class="n">3</span><span class="t">T = (n−1)·M/(n·B)</span><span class="d">root 链路串行接收 (n−1) 份</span></div>
        ...
        <div class="bw-concl">
          ...
          <div class="h">Gather 是 Scatter 的逆运算，公式完全对称。root 入向链路接收 (n−1)/n·M。</div>
        </div>
        <div class="bw-legend">
          <span>每 rank 贡献 <b>base=16MB</b></span>
          <span>root 接收份数 <b>n−1=7</b></span>
          <span>root 总接收 <b>7×16=112MB</b></span>
...
```c
void GatherGetBw(size_t count, int typesize, double sec,
...
- `count` = paramcount = base = 每 rank **贡献（发送）**字节数
- `baseBw` = base × n / T = root 接收总量 / T
- `factor` = (n−1)/n（与 Scatter 完全相同）

## 二、参数含义（用户 -b 128MB, n = 8）

GatherGetCollByteCount 中：`sendcount = count/nranks`，root 收集 n 份。
...
| recvcount (root) | = base×n | 128 MB（root 接收总量）|
| baseBw 对应数据 | paramcount×n | 128 MB |

## 三、算法推导（树形/环形 Gather）

Gather 是 all→1 收集：每 rank 向 root 发 base，root 入向链路是瓶颈。
...
graph TD
    ...
    G2 -->|base| R
    G3["..."] -->|base| R
    G7 -->|base| R["Root G0<br/>收 8×16MB=128MB"]
```

- root 需收 n−1 个其他 rank 各 base，共 (n−1)×base = 112MB
- root 入向链路串行接收，T = (n−1)×base/B
- 总时间：
...
设 B = 单向链路带宽，M = 128MB（root 接收总量 = base×n）。

| 步骤 | 公式 | 代入 n=8, M=128MB | 结果 |
|------|------|-------------------|------|
| 1. 每 rank 贡献 | base = M/n | 128/8 | 16 MB |
| 2. root 接收份数 | n−1 | 8−1 | 7 |
...
- Gather 是 Scatter 的逆运算，公式完全对称
- root 入向链路接收 (n−1)×base = (n−1)/n × M
- factor = (n−1)/n 把 algBw（基于 M）归一化到 root 链路实际负载

> **镜像关系**：Scatter（root 发 (n−1)×base）↔ Gather（root 收 (n−1)×base），两者 factor、baseBw、busBw 上限完全相同。
